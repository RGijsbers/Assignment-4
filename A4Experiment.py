...
from A4Agents import DynaAgent, A2CAgent, QLearningAgent
...
def run_repetitions(policy, n_repetitions, n_timesteps, smoothing_window, learning_rate, gamma,
    ...
    if policy == 'A2C':
        pi = A2CAgent(env.n_states,env.n_actions,learning_rate,gamma) #initialize Prioritized Sweeping agent
    if policy == 'Q':
        pi = QLearningAgent(env.n_states, env.n_actions, learning_rate, epsilon, gamma)
